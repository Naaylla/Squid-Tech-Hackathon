...
export default function Navbar() {
  ...
  return (
    <div className={`w-full flex fixed top-0 left-0 bg-teal-700 text-white justify-between items-center px-4 md:px-14 py-5 z-40 ${showBorderBottom ? "border-b-2" : ""}`}>
      <img src={TreeIcon} className="w-10" alt="Logo" />
      ...
      <div className={`flex-col md:flex md:flex-row md:items-center font-semibold gap-4 md:gap-20 ${menuOpen ? "flex" : "hidden"}`}>
        ...
        <div className="relative w-auto">
          <input type="text" placeholder="Rechercher..." className="border border-gray-300 bg-white text-black rounded-full pl-10 pr-3 py-2 w-80 focus:outline-none focus:ring-teal-500 focus:border-teal-500 sm:text-sm" />
        </div>

        <div className="flex gap-4 justify-end items-center">
          <div className="relative" ref={friendsListRef}>
            <m.button className="p-1" onClick={handleFriendsClick}>
              <img src={FriendsIcon} className="w-8 text-white" alt="Friends" />
            </m.button>
            {friendsOpen && <FriendsList />}
          </div>

          <button className="p-1" onClick={handleNotificationClick}>
            <svg className="w-8 h-8 text-white dark:text-white" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24">
          ...
          <m.button className="p-1" onClick={() => { window.location.href = "/profile"; }}>
            <img src={UserCircleIcon} className="w-8 text-white" alt="Profile" />
          </m.button>
        </div>
      </div>
    </div>
